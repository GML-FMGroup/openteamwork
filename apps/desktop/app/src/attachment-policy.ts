...
const MODERN_DOCUMENT_EXTENSIONS = [".docx", ".xlsx", ".csv", ".pdf", ".pptx"];
...
const SUPPORTED_EXTENSIONS = new Set([
  ...MODERN_DOCUMENT_EXTENSIONS,
  ...IMAGE_EXTENSIONS,
  ...TEXT_EXTENSIONS,
]);
const LEGACY_OFFICE_EXTENSIONS = new Set([".doc", ".xls", ".ppt"]);

export const ATTACHMENT_ACCEPT = [...MODERN_DOCUMENT_EXTENSIONS, ...IMAGE_EXTENSIONS, ...TEXT_EXTENSIONS].join(",");
...
export function attachmentPreflightError(file: Pick<File, "name" | "size">): string | null {
  ...
  if (LEGACY_OFFICE_EXTENSIONS.has(extension)) {
    return `${file.name} uses a legacy Office format. Convert it to DOCX, XLSX, or PPTX first.`;
  }
