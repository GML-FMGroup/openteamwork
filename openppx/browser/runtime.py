"""Minimal browser runtime abstraction for openppx native browser tool.
...
_SUPPORTED_PROFILES = {"openppx", "chrome"}
_OPENPPX_ACTIONS = [
    "status",
    "start",
    "stop",
...
class InMemoryBrowserRuntime:
    ...
    def _resolve_profile(self, profile: str | None) -> str:
        resolved = (profile or "").strip().lower() or "openppx"
        if resolved not in _SUPPORTED_PROFILES:
            raise BrowserRuntimeError("unknown profile; supported profiles are openppx, chrome")
        return resolved
