"""Persistent config support for openppx."""
...
def get_data_dir() -> Path:
    """Return the data directory used by openppx."""
...
def _normalize_runtime_memory_dir_override(
    ...
    *,
    ...
    global `~/.openppx/workspace/memory`, which breaks per-agent isolation.
    ...
    legacy_global_default = (Path.home() / ".openppx" / "workspace" / "memory").resolve(strict=False)
    try:
