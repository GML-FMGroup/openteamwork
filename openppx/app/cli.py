"""Minimal CLI helpers for openppx."""
...
def _gui_execution_path_hint(
    *,
    ...
    if builtin_tools_enabled and not gui_mcp_configured:
        return (
            "builtin_only",
            "Only builtin GUI tools are enabled. Configure `tools.mcpServers.openppx_gui` to move GUI execution into MCP.",
        )
...
def _resolve_bridge_source_dir() -> Path:
    ...
    monorepo_bridge = Path(__file__).resolve().parents[2] / "openppx" / "bridge"
    ...
    raise RuntimeError(
        ...
        "Set OPENPPX_WHATSAPP_BRIDGE_SOURCE or include openppx/bridge resources."
    )
...
def _default_agent_workspace(agent_name: str) -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix=f"openppx-{agent_name}-")).resolve()
...
def _render_init_sections_with_rich(
    *,
    ...
    console.print(Panel(summary, title="[bold]OpenPPX Init[/bold]", border_style="cyan"))
...
def _render_install_welcome_with_rich(
    *,
    ...
    console.print(Panel(table, title="[bold]OpenPPX Install Wizard[/bold]", border_style="cyan"))
    return True
...
def _gateway_service_exec_argv(channels: str) -> tuple[str, list[str]]:
    ...
    return sys.executable, ["-m", "openppx.app.cli", "gateway", "run", "--channels", channels]
...
def _cmd_gateway_service_install(*, force: bool, channels: str, enable: bool) -> int:
    ...
    service_name = gateway_service_name("openppx")
    ...
    if manager == "launchd":
        ...
        manifest = render_systemd_unit(
            description="OpenPPX Gateway Service",
